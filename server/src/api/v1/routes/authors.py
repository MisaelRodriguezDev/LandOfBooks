...
@router.get('', response_model=ApiResponse[list[AuthorOut]], responses=GET_RESPONSES)
...
@router.get('/admin', response_model=ApiResponse[list[AuthorOut]], responses=GET_RESPONSES)
def get_all_authors(
    service: AuthorService = Depends(get_service(AuthorService)),
    _ = Depends(is_librarian_or_admin)
):
    authors = service.get_all_authors_from_admin()
    return ApiResponse(data=authors)

@router.get('/{id}', response_model=ApiResponse[AuthorOut], responses=GET_RESPONSES)
...
def create_author(data: AuthorIn ,service: AuthorService = Depends(get_service(AuthorService)), _ = Depends(is_librarian_or_admin)):
    response = service.create_author(data)
    return ApiResponse(data=response)

@router.patch('/{id}', response_model=ApiResponse, responses=PUT_RESPONSES)
def update_author(id: UUID, data: AuthorUpdate, service: AuthorService = Depends(get_service(AuthorService)), _ = Depends(is_librarian_or_admin)):
    author = service.update_author(id, data)
    return ApiResponse(data=author)

@router.delete('/{id}', response_model=ApiResponse[str], responses=DELETE_RESPONSES)
def delete_author(id: UUID, service: AuthorService = Depends(get_service(AuthorService)), _ = Depends(is_librarian_or_admin)):
